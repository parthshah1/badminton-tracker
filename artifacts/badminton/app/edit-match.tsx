...
export default function EditMatchScreen() {
  ...
  // If playedAt is outside the 30-day range, include it
  const playedAtDay = new Date(playedAt);
  playedAtDay.setHours(0, 0, 0, 0);
  const inRange = dateDays.some(d => isSameDay(d, playedAtDay));
  const displayDays = inRange ? dateDays : [...dateDays, playedAtDay];
  ...
  // Fetch the existing match to pre-fill form
  useEffect(() => {
    if (!id) return;
    ...
    fetch(`${BASE}/api/matches/${id}`)
      ...
      .then((match: any) => {
        ...
        const t1 = match.players.filter((p: any) => p.team === 1).map((p: any) => p.playerId);
        const t2 = match.players.filter((p: any) => p.team === 2).map((p: any) => p.playerId);
        setTeam1Ids(t1);
        setTeam2Ids(t2);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 16, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Match</Text>
        <Pressable
      ...
      >
          ...
          <View style={[styles.segmented, { backgroundColor: colors.backgroundSecondary }]}>
            {(["singles", "doubles"] as MatchType[]).map((type) => (
              <Pressable
                ...
                style={[styles.segment, matchType === type && { backgroundColor: colors.card }]}
              >
                <Text style={[styles.segmentText, { color: matchType === type ? colors.text : colors.textMuted }]}>
            ...
            <View style={styles.scoreTeamBlock}>
              <View style={styles.scoreAvatarRow}>
                {team1Players.length > 0 ? (
                  team1Players.map(p => <PlayerAvatar key={p.id} name={p.name} color={p.avatarColor} size={28} fontSize={11} />)
                ) : (
                  <View style={[styles.emptyAvatar, { backgroundColor: colors.backgroundSecondary }]}>
                    <Text style={[styles.emptyAvatarText, { color: colors.textMuted }]}>{matchType === "singles" ? "P1" : "T1"}</Text>
                  </View>
                )}
              </View>
              <TextInput
                ...
                onChangeText={(v) => {
                  const n = parseInt(v);
                  if (v === "" || (!isNaN(n) && n <= MAX_SCORE)) setTeam1Score(v);
                }}
            ...
            <View style={styles.scoreTeamBlock}>
              <View style={styles.scoreAvatarRow}>
                {team2Players.length > 0 ? (
                  team2Players.map(p => <PlayerAvatar key={p.id} name={p.name} color={p.avatarColor} size={28} fontSize={11} />)
                ) : (
                  <View style={[styles.emptyAvatar, { backgroundColor: colors.backgroundSecondary }]}>
                    <Text style={[styles.emptyAvatarText, { color: colors.textMuted }]}>{matchType === "singles" ? "P2" : "T2"}</Text>
                  </View>
                )}
              </View>
              <TextInput
                ...
                onChangeText={(v) => {
                  const n = parseInt(v);
                  if (v === "" || (!isNaN(n) && n <= MAX_SCORE)) setTeam2Score(v);
                }}
                keyboardType="number-pad"
                placeholder="0"
                placeholderTextColor={colors.textMuted}
        ...
        {/* Player selection */}
        <View style={styles.field}>
          <Text style={[styles.label, { color: colors.textSecondary }]}>Select Players (tap to assign teams)</Text>
            ...
            <View style={styles.playerPicker}>
              <View style={styles.teamsHeader}>
                <View style={[styles.teamLabel, { backgroundColor: colors.team1 + "22" }]}>
                  ...
                  <Text style={[styles.teamCount, { color: colors.team1 }]}>{team1Ids.length}/{maxPerTeam}</Text>
                </View>
                <View style={[styles.teamLabel, { backgroundColor: colors.team2 + "22" }]}>
                  ...
                  <Text style={[styles.teamCount, { color: colors.team2 }]}>{team2Ids.length}/{maxPerTeam}</Text>
                </View>
              </View>
          ...
          >
            {displayDays.map((d, i) => {
              ...
              return (
                <Pressable
                  key={i}
                  onPress={() => { setPlayedAt(new Date(d)); Haptics.selectionAsync(); }}
                  style={[styles.dateChip, {
                    backgroundColor: selected ? colors.tint : colors.card,
                    borderColor: selected ? colors.tint : colors.border,
                  }]}
                >
                  <Text style={[styles.dateChipText, { color: selected ? "#fff" : colors.text }]}>
                    {formatChip(d)}
                  </Text>
                  {i === 0 && (
...
const styles = StyleSheet.create({
  ...
  saveBtn: { fontSize: 17, fontFamily: "Inter_700Bold" },
  field: { gap: 10 },
  label: { fontSize: 13, fontFamily: "Inter_500Medium", letterSpacing: 0.3 },
  segmented: { flexDirection: "row", borderRadius: 12, padding: 4, gap: 2 },
  ...
  scoreTeamBlock: { flex: 1, alignItems: "center", gap: 10 },
  scoreAvatarRow: { flexDirection: "row", gap: 4, minHeight: 28 },
  emptyAvatar: { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  emptyAvatarText: { fontSize: 10, fontFamily: "Inter_600SemiBold" },
  scoreInput: {
    width: "100%", textAlign: "center", fontSize: 48, fontFamily: "Inter_700Bold",
    borderRadius: 16, borderWidth: 2, paddingVertical: 16,
  },
  scoreDivider: { fontSize: 18, fontFamily: "Inter_500Medium", marginTop: 28 },
  tieWarning: { fontSize: 13, fontFamily: "Inter_500Medium", textAlign: "center" },
  labelRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  charCount: { fontSize: 12, fontFamily: "Inter_400Regular" },
  playerPicker: { gap: 14 },
  teamsHeader: { flexDirection: "row", gap: 8 },
  teamLabel: { flex: 1, flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10 },
  teamDot: { width: 8, height: 8, borderRadius: 4 },
  teamLabelText: { fontSize: 13, fontFamily: "Inter_600SemiBold", flex: 1 },
  teamCount: { fontSize: 12, fontFamily: "Inter_500Medium" },
  playerGrid: { gap: 8 },
  playerPickerItem: { flexDirection: "row", alignItems: "center", gap: 12 },
  teamBtn: { width: 40, height: 40, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  teamBtnText: { fontSize: 13, fontFamily: "Inter_700Bold" },
  playerPickerCenter: { flex: 1, flexDirection: "row", alignItems: "center", gap: 12 },
  playerPickerName: { fontSize: 15, fontFamily: "Inter_500Medium", flex: 1 },
  notesInput: {
    borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12,
    fontSize: 15, fontFamily: "Inter_400Regular", minHeight: 80, textAlignVertical: "top",
  },
  errorText: { fontSize: 14, fontFamily: "Inter_400Regular", textAlign: "center" },
  dateChip: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 14, paddingVertical: 9, borderRadius: 20, borderWidth: 1.5 },
